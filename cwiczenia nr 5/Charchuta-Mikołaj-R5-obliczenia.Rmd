...
W tym dokumencie znajdują się rozwiązania zadań z korelacji i regresji (R3). Dla każdego zadania zamieszczam: dane, obliczenia współczynnika korelacji (gdzie dotyczy), test statystyczny, wykresy pomocnicze oraz estymację regresji liniowej (jeśli wymagana). Domyślnie używam testu Pearsona dla korelacji i testu t / lm dla regresji; gdzie istnieją przesłanki do użycia metod nieparametrycznych (np. małe n lub brak normalności), podaję alternatywy.
...
Uwaga: zadanie mówi, że 60 konsumentów oceniało obie kawy. Aby przetestować zależność ocen potrzebujemy parowanych (wspólnych) danych (ocena kawy A i ocena kawy B dla tego samego konsumenta). W treści podano jedynie rozkłady punktów dla każdej kawy, a nie rozkłady skojarzone (brak tablicy 60 par). Zatem z samych marginesów nie można obliczyć korelacji czy testu zależności par.

Propozycje:
- Jeśli masz surowe dane (wektor ocen A i B po 60 obserwacjach), użyj testu korelacji rang Spearmana lub tau Kendalla: cor.test(A, B, method = "spearman").
- Jeśli masz pełną macierz kontyngencji (ocena A vs ocena B), możesz wykonać test chi-kwadrat niezależności lub test zgodności.

Podaj proszę surowe pary ocen lub macierz 5x5 ocen, a wstawię analizę.
...
Jeżeli chcesz, uruchomię te bloki w Twoim środowisku R i wkleję wyniki testów oraz wykresy (mogę też dodać interpretacje numeryczne przy każdym zadaniu). Napisz czy mam uruchomić i wkleić wyniki tutaj, czy wystarczy, że zostawię Rmd gotowy do uruchomienia w RStudio.
---
title: "Regresja"
author: "Charchuta — Mikołaj"
output: html_document
